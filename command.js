...
registerPlugin({
    ...
    class StringArgument extends Argument {
        ...
        validateString(arg, rest) {
            if (this.uppercase)
                arg = arg.toUpperCase();
            if (this.lowercase)
                arg = arg.toLowerCase();
            if (this.minlen !== null && this.minlen > arg.length)
                throw new ParseError(`String length not greater or equal! Expected at least ${this.minlen}, but got ${arg.length}`, this);
            if (this.maxlen !== null && this.maxlen < arg.length)
                throw new ParseError(`String length not less or equal! Maximum ${this.maxlen} chars allowed, but got ${arg.length}`, this);
            if (this.whitelistWords !== null && !this.whitelistWords.includes(arg))
                throw new ParseError(`Invalid Input for ${arg}. Allowed words: ${this.whitelistWords.join(", ")}`, this);
            if (this.regex !== null && !this.regex.test(arg))
                throw new ParseError(`Regex missmatch, the input '${arg}' did not match the expression ${this.regex.toString()}`, this);
            return [arg, rest];
        }
        /**
         ...
         */
        whitelist(words) {
            if (!Array.isArray(this.whitelistWords))
                this.whitelistWords = [];
    ...
    class NumberArgument extends Argument {
        ...
        validate(args) {
            ...
            if (this.minLen !== null && this.minLen > num)
                throw new ParseError(`Number not greater or equal! Expected at least ${this.minLen}, but got ${num}`, this);
            if (this.maxLen !== null && this.maxLen < num)
                throw new ParseError(`Number not less or equal! Expected at least ${this.maxLen}, but got ${num}`, this);
            if (this.int && num % 1 !== 0)
    ...
    class GroupArgument extends Argument {
        ...
        validateOr(args) {
            ...
            if (!valid)
                throw new ParseError(`No valid match found`, this);
        ...
        addArgument(arg) {
            if (typeof arg === "function")
                arg = arg(Argument.createArgumentLayer());
            if (!(arg instanceof Argument))
                throw new Error(`Typeof arg should be function or instance of Argument but got ${arg}`);
    ...
    class Throttle {
        ...
        timeTillNextCommand(client) {
            if (this.throttled[client.uid()] === undefined)
                return 0;
    ...
    class CommandGroup extends BaseCommand {
        ...
        async hasPermission(client) {
            if (!await this.isAllowed(client))
                return false;
            if (this.execHandler.length > 0)
                return true;
        ...
        findCommandByName(name) {
            name = name.toLowerCase();
            if (name.length === 0)
                throw new CommandNotFoundError(`No subcommand specified for Command ${this.getFullCommandName()}`);
            const cmd = this.commands.find(c => c.getCommandNames().includes(name));
            if (!cmd)
                throw new CommandNotFoundError(`Command with name "${name}" has not been found on Command ${this.getFullCommandName()}!`);
            return cmd;
        }
        /**
    ...
    class Collector {
        ...
        static isValidCommandName(name) {
            if (typeof name !== "string")
                throw new Error("Expected a string as command name!");
            if (name.length < 1)
                throw new Error(`Command should have a minimum length of 1!`);
            if ((/\s/).test(name))
        ...
        getAvailableCommandsByPermission(client) {
            return Collector.checkPermissions(this.commands.filter(cmd => cmd.isEnabled()), client);
        }
        /**
        ...
        isPossibleCommand(text) {
            if (text.startsWith(Collector.getCommandPrefix()))
                return true;
        ...
        isSaveCommand(cmd) {
            ...
            if (this.getAvailableCommands(cmd).length > 0)
                return false;
            return true;
        }
    }
    ...
    collector.registerCommand("help")
        ...
        await Promise.all(cmds.map(async (cmd) => {
            if (cmd instanceof CommandGroup) {
                if (cmd.getFullCommandName().length > length)
                    length = cmd.getFullCommandName().length;
    ...
    collector.registerCommand("man")
        ...
        if (cmds.length === 0)
            return reply(`No command with name ${format.bold(command)} found! Did you misstype the command?`);
    ...
    function messageHandler(ev) {
        ...
        if (!ev.client)
            return debug(DEBUG.WARNING)("client is undefined");
        //do not do anything when the bot sends a message
        if (ev.client.isSelf())
            return debug(DEBUG.VERBOSE)("Will not handle messages from myself");
        //check if it is a possible command
        if (!collector.isPossibleCommand(ev.text))
            return debug(DEBUG.VERBOSE)("No possible valid command found!");
